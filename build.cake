  ...
  .Does(() =>
{
  ...
  var file = new FilePath("src/Nancy.Bootstrappers.Windsor/project.json");
  ...
  .Does(() =>
{
  ...
  else
  {
    ...
    using(var process = StartAndReturnProcess("mono", new ProcessSettings{Arguments =
      xunit + " " +
      "test/Nancy.Bootstrappers.Windsor.Tests/bin/"+configuration+"/net452/unix-x64/Nancy.Bootstrappers.Windsor.Tests.dll"}))
    {
      process.WaitForExit();
...
Task("Package")
  ...
  .Does(() =>
{
  var package = outputPackages + File("Nancy.Bootstrappers.Windsor-Latest.zip");
